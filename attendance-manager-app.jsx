...
.am-shell { position:absolute; inset:0; display:flex; flex-direction:column; }
...
/* App bar */
.am-appbar {
  display:flex; align-items:center; gap:4px; padding: 14px 8px 14px 8px;
...
/* FAB */
.am-fab {
  position:absolute; right:20px; bottom: calc(20px + env(safe-area-inset-bottom, 0px)); z-index:8;
...
.am-tab-pane { position:absolute; inset:0; overflow-y:auto; overflow-x:hidden; -webkit-overflow-scrolling:touch; transition: opacity 180ms ease; }
...
function AttendanceLoggingScreen() {
  const { classSessions, attendanceRecords, subjectById, markAttendance } = useAttendanceData();
    ...
    <div className="am-screen">
      ...
      <div style={{ padding: '8px 16px' }}>
        ...
        {todaysSessions.length === 0 ? (
          ...
          <SessionCard key={item.id} session={item} subjectLabel={subjectLabel(item.subjectId)}
...
function CalendarScreen() {
  const { classSessions, attendanceRecords, subjectById } = useAttendanceData();
  const today = startOfDay(new Date());
    ...
    <div className="am-screen no-bottom-pad" style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
      ...
      <div className="am-scroll-area" style={{ padding: '8px 16px' }}>
        {selectedItems.length === 0 ? (
          ...
          <SessionCardConnected key={item.session.id} session={item.session} subjectLabel={subjectLabel(item.session.subjectId)} />
        ))}
      </div>
    </div>
  );
...
function HistoryScreen({ onBack }) {
  const { classSessions, attendanceRecords, subjects, subjectById, activeSubjects, markAttendance } = useAttendanceData();
...
function BackupScreen({ onBack }) {
  ...
  function backupNow() {
    try {
      ...
      downloadTextFile(`attendance-manager-backup-${dateKey(new Date())}.json`, json);
    } catch (e) {
...
function ExportScreen({ onBack }) {
  const { classSessions, attendanceRecords, subjects, settings } = useAttendanceData();
